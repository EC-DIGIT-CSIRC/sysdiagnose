...
class SummaryAnalyser(BaseAnalyserInterface):
    ...
    def execute(self):
        ...
        mobilecal_result = PlistParser.parse_file(
            os.path.join(self.case_data_subfolder, "logs/CalendarPreferences/com.apple.mobilecal.plist")
        )
        try:
        ...
        if not trans_result:
            ...
            try:
                ...
                for _key, device in trans_result["stateMachine"]["devices"].items():
                    result.append(
                        f"| {device['serial']} | {device['model']:16} | {device['osVersion']:9} | {device['name']:20} |"
                    )
            except KeyError:
